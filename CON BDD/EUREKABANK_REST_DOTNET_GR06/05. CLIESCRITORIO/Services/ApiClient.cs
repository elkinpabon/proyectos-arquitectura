...
namespace CLIESCRITORIO.Services;
...
public class ApiClient
{
    ...
    public async Task<Resultado> Depositar(string c, string m, string mo) { var r = await _http.PostAsJsonAsync("api/cuenta/depositar", new { cuenta = c, monto = m, moneda = mo }); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
    public async Task<Resultado> Retirar(string c, string m, string mo) { var r = await _http.PostAsJsonAsync("api/cuenta/retirar", new { cuenta = c, monto = m, moneda = mo }); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
    public async Task<Resultado> ConsultarSaldo(string c) { var r = await _http.GetAsync($"api/cuenta/saldo/{c}"); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
    public async Task<Resultado> Transferir(string o, string d, string m, string mo) { var r = await _http.PostAsJsonAsync("api/cuenta/transferir", new { origen = o, destino = d, monto = m, moneda = mo }); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
    ...
    public async Task<Resultado> RegistrarCuenta(string c, string m) { var r = await _http.PostAsJsonAsync("api/cuenta", new { cliente = c, moneda = m }); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
    public async Task<Resultado> EliminarCuenta(string c) { var r = await _http.DeleteAsync($"api/cuenta/{c}"); return await r.Content.ReadFromJsonAsync<Resultado>() ?? new Resultado(); }
