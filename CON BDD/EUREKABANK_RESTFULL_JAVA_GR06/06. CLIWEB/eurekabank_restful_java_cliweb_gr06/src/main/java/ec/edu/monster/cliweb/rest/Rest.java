package ec.edu.monster.cliweb.rest;
...
public final class Rest {
    ...
    private static String url(String path) {
        return ServidorConfig.BASE + path;
    }
    ...
    public static String getText(String path) {
        return enviar(HttpRequest.newBuilder(URI.create(url(path)))
                .GET().build());
    }

    private static jakarta.json.JsonStructure parse(String body) {
    ...
    public static JsonArray getArray(String path) {
        jakarta.json.JsonStructure s = parse(getText(path));
        return s == null ? Json.createArrayBuilder().build() : (JsonArray) s;
    }

    public static JsonObject post(String path, String jsonBody) {
    ...
    /* ---- helpers de lectura JSON tolerantes a nulos/ausentes ---- */
